...
var SNAView = class extends import_obsidian.ItemView {
  constructor(leaf, plugin) {
    ...
    __publicField(this, "currentNormalizationOptions");
    this.plugin = plugin;
...
var GraphAnalyzer = class {
  constructor(plugin, calculator) {
    ...
    __publicField(this, "layoutEngine");
    this.plugin = plugin;
  ...
  getGraphViewFilter() {
    try {
      ...
      if (graphLeaves && graphLeaves.length > 0) {
        const graphState = graphLeaves[0].view;
  ...
  extractGraphData() {
    try {
      ...
      new import_obsidian2.Notice(`Error extracting graph data: ${errorMessage}`);
...
`;
  ...
  async exportResults() {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      new import_obsidian2.Notice(`Error exporting results: ${errorMessage}`);
    }
  }
};
...
var SNAPlugin = class extends import_obsidian3.Plugin {
  constructor() {
    ...
    __publicField(this, "snaCalculator");
    __publicField(this, "graphAnalyzer");
  }
  async onload() {
  ...
  async activateSNAView() {
    ...
    let leaf = workspace.getLeavesOfType(VIEW_TYPE_SNA).length > 0 ? workspace.getLeavesOfType(VIEW_TYPE_SNA)[0] : workspace.getLeaf(false);
...
var SNASettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Minimum Link Weight").setDesc("Minimum number of mentions to show a link (when weighted)").addSlider(
      (slider) => slider.setLimits(1, 10, 1).setValue(this.plugin.settings.minLinkWeight).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Metrics").setHeading();
